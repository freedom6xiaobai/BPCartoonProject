...
class BPFindModuleType1Cell: UITableViewCell, UICollectionViewDelegate,
UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        collectionView?.register(BPFindModuleType1ContentCell.self, forCellWithReuseIdentifier: "BPFindModuleType1ContentCell")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell:BPFindModuleType1ContentCell = collectionView.dequeueReusableCell(withReuseIdentifier: "BPFindModuleType1ContentCell", for: indexPath) as! BPFindModuleType1ContentCell

        if self.moduleModel!.items!.count > 0 {
            let model = self.moduleModel?.items?[indexPath.row]
            cell.itemsModel = model?[Int(arc4random() % UInt32(model!.count))]
        }else{
            cell.itemModel = self.moduleModel?.item?.first
        }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if self.moduleModel!.items!.count > 0 {
            return CGSize(width: KScreenWidth / 2 - KScale(10), height: KScale(120))
        }else{
...
class BPFindModuleType1ContentCell: UICollectionViewCell {
    //
